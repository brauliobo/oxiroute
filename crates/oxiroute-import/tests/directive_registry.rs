...
fn reports_truthful_key_and_form_statuses() {
    ...
    assert_eq!(report.directive_status.partial, 35);
    assert_eq!(report.directive_status.disable_only, 0);
    assert_eq!(report.directive_status.parsed_only, 68);
    assert_eq!(report.directive_status.source_no_op, 1);
    assert_eq!(report.directive_status.source_bug, 1);
    assert_eq!(report.directive_status.deprecated, 1);
    assert_eq!(report.directive_status.platform_limited, 4);
    assert_eq!(report.form_status.enforced, 48);
    assert_eq!(report.form_status.partial, 0);
    assert_eq!(report.form_status.disable_only, 5);
    assert_eq!(report.form_status.parsed_only, 33);
    assert_eq!(report.form_status.source_no_op, 1);
    assert_eq!(report.form_status.source_bug, 1);
    assert_eq!(report.form_status.deprecated, 1);
    assert_eq!(report.form_status.platform_limited, 5);
...
#[test]
fn registry_uses_the_characterized_form_inventory() {
    let source = include_str!("../src/nginx/rtmp_directives.rs");
    let form_arrays = source
        .lines()
        .filter(|line| line.starts_with("const ") && line.contains("_FORMS:"))
        .count();
    let explicit_forms = directive_specs()
        .iter()
        .map(|spec| spec.runtime_forms.len())
        .sum::<usize>();

    assert_eq!(form_arrays, 33);
    assert_eq!(explicit_forms, 94);

    let snapshot = directive_specs()
        .iter()
        .fold(String::new(), |mut output, spec| {
            writeln!(output, "{spec:?}").expect("write registry snapshot");
            output
        });
    ...
    assert_eq!(fingerprint, 17_287_968_383_810_029_747);
}
...
fn spec(name: &str) -> &'static oxiroute_import::nginx::DirectiveSpec {
    directive_specs()
        .iter()
