...
fn imported_http_services_enable_automatic_response_headers_in_canonical_rendering() {
    ...
    let source = serde_json::to_string(config).expect("serialized nginx import");
...
fn lowers_proxy_pass_uri_replacement_and_accepts_explicit_expires_off() {
    ...
    assert!(
        serde_json::to_string(config)
            .expect("serialized rewritten proxy config")
...
fn lowers_explicit_nginx_proxy_version_with_canonical_defaults() {
    ...
    assert_eq!(report.summary().http_services, 1);
    let config = report
        .validated()
        .map(oxiroute_config::ValidatedConfig::as_draft)
        .expect("canonical proxy defaults");
    let HttpRouteAction::Proxy { policy, .. } = &config.http_services[0].routes[0].action else {
        panic!("proxy route");
    };
    assert_eq!(
...
fn default_server_retains_named_selectors_before_its_fallback() {
    ...
    let config = report
        .validated()
        .map(oxiroute_config::ValidatedConfig::as_draft)
        .expect("default server host selectors");
...
fn lowers_inherited_nginx_tls_policy_exactly() {
    ...
    let config = report
        .validated()
        .map(oxiroute_config::ValidatedConfig::as_draft)
        .expect("canonical nginx TLS policy");
    assert!(report.blocked_services.is_empty());
    assert_eq!(
...
fn unix_http_listener_is_retained_without_a_socket_placeholder() {
    ...
    assert!(report.blocked_services.is_empty());
    assert!(matches!(
        report.validated().map(oxiroute_config::ValidatedConfig::as_draft).unwrap().listeners[0].bind,
...
fn exact_location_fixed_and_redirect_actions_finalize_without_placeholders() {
    ...
    let config = report
        .validated()
        .map(oxiroute_config::ValidatedConfig::as_draft)
        .expect("fixed/static nginx config");
    assert!(matches!(
...
fn lowers_static_index_behavior_into_canonical_static_routes() {
    for index in ["", "index home.html;"] {
        ...
        assert!(report.blocked_services.is_empty());
        assert_eq!(report.summary().upstream_pools, 0);
...
fn bare_nginx_proxy_timeouts_preserve_seconds_for_slow_upstreams() {
    ...
    let config = report
        .validated()
        .map(oxiroute_config::ValidatedConfig::as_draft)
        .expect("bare timeout proxy config");
...
fn blocks_proxy_pass_header_date_that_pingora_replaces() {
    ...
    assert_eq!(report.summary().listeners, 0);
    assert!(report.diagnostics.iter().any(|diagnostic| {
        diagnostic.stage() == DiagnosticStage::Lower
...
fn lowers_inherited_add_header_for_every_action_and_rejects_dynamic_values() {
    ...
    let config = report
        .validated()
        .map(oxiroute_config::ValidatedConfig::as_draft)
        .expect("literal add_header policy");
    let routes = &config.http_services[0].routes;
...
fn lowers_literal_response_directives_with_order_statuses_provenance_and_rendering() {
    ...
    let config = report
        .validated()
        .map(oxiroute_config::ValidatedConfig::as_draft)
        .expect("literal response directives");
    ...
    let rendered = serde_json::to_string(config).expect("serialized literal response directives");
...
fn certbot_server_if_precedes_only_its_exact_host_return_fallback() {
    ...
    let config = report
        .validated()
        .map(oxiroute_config::ValidatedConfig::as_draft)
        .expect("Certbot server return routes");
    let routes = &config.http_services[0].routes;
...
fn nginx_host_fallback_serializes_ipv6_as_an_http_authority() {
    ...
    let config = report
        .validated()
        .map(oxiroute_config::ValidatedConfig::as_draft)
        .expect("IPv6 nginx host fallback");
    let HttpRouteAction::Proxy { policy, .. } = &config.http_services[0].routes[0].action else {
        panic!("proxy route");
    };
    assert_eq!(
...
fn blocks_unmodeled_try_files_reroutes_but_models_exact_index_policy_reselection() {
    ...
    let config = report
        .validated()
        .map(oxiroute_config::ValidatedConfig::as_draft)
        .expect("exact index reroute is represented");
    let HttpRouteAction::StaticFiles {
