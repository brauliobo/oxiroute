...
`https://github.com/brauliobo/oxiroute/releases/download/v0.2.2/oxiroute-0.2.2.tar.gz`. The asset is a
deterministic archive of Git-tracked files with the `oxiroute-0.2.2/` prefix and without this
...
```sh
./build-local.sh
./build-local.sh /path/to/oxiroute-0.2.2.tar.gz --cleanbuild
```
