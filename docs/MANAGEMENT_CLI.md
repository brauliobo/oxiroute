...
| Config | native import | Config generation | `oxiroute import nginx|haproxy ...`; import remains an offline evidence-producing operation. |
...
| Cache | purge | Intentionally unsupported | Canonical cache policy exists, but the server rejects cache-backed routes because no cache runtime is active. No purge command or fake success is exposed. |
