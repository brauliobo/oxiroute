...
type PemItem = (SectionKind, Vec<u8>);
...
fn load_pem_file<P>(path: P) -> Result<Vec<PemItem>>
where
    P: AsRef<Path>,
{
    <PemItem>::pem_reader_iter(load_file(path)?)
        .map(|item_res| {
            item_res.or_err(
...
where
    P: AsRef<Path>,
{
    for (kind, content) in load_pem_file(path)? {
        if kind == SectionKind::EchConfigList {
            continue;
        }
...
pub fn load_certs_and_key_files<'a>(
    ...
    let certs = certs_file
        ...
        .filter_map(|(kind, content)| {
            (kind == SectionKind::Certificate).then(|| CertificateDer::from(content))
        })
        .collect::<Vec<_>>();
    ...
    let private_key_opt = key_file
        ...
        .filter_map(|(kind, key)| match kind {
            SectionKind::RsaPrivateKey => Some(PrivateKeyDer::Pkcs1(key.into())),
            SectionKind::PrivateKey => Some(PrivateKeyDer::Pkcs8(key.into())),
            SectionKind::EcPrivateKey => Some(PrivateKeyDer::Sec1(key.into())),
            _ => None,
        })
        .next();
