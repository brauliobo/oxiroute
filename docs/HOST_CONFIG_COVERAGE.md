...
| HN-08 | Fixed-IP HTTP proxy destinations | partial | Runtime and strict fragment lowering exist, including canonical static URI-prefix replacement; the audited service remains blocked. |
...
| HN-16 | Public static document root and index handling | partial | Descriptor-pinned root/alias mapping, ordered try-files, indexes, autoindex, MIME, headers, ETag on/off, explicit `expires off`, errors, and ranges run on wire; the audited alias/try-files/ETag-off shape lowers, while broader nginx location reselection remains blocked. |
...
| HN-20 | Mixed buffered and unbuffered proxy paths | partial | Request buffering reads the bounded body before upstream connection; response buffering admits only bounded fixed-length responses and rejects unbounded or oversized origins; native lowering retains nginx's default. |
...
| HH-14 | Passive server observation and bounded retry-on | covered | `observe layer4/layer7`, common `error-limit`, `on-error mark-down`, and `retry-on conn-failure/conn-refused` lower to one canonical per-pool policy; response/status retry tokens and non-uniform server policies remain blocking. |
