...
use lua_support::{load_lua, render_lua, render_lua_error};
use oxiroute_config::{
    AccessLogPolicy, AlpnProtocol, Certificate, CertificateSource, ConfigDraft, ConfigError,
    DnsResolutionPolicy, DownstreamTimeoutPolicy, HealthCheck, HealthCheckType, HealthStartup,
    HttpAccessPolicy, HttpCookiePathRewrite, HttpHostSelector, HttpLiteralHeader, HttpPathSelector,
    ...
    PassiveHealthPolicy, PassiveObserve, PassiveOnError, Protocol, RtmpAccessPolicy,
    RtmpApplication, RtmpExecEnvironment, RtmpExecFilesystemPolicy, RtmpExecMode,
    ...
    RtmpService, RtmpSessionCeilings, TlsClientAuthMode, TlsClientAuthPolicy, TlsPolicy,
    TlsProfile, TlsSessionCache, TlsVersion, UdpPolicy, UpstreamAlgorithm, UpstreamConnectionReuse,
    UpstreamEndpoint, UpstreamPool, UpstreamServer, UpstreamTls,
};

const MAX_SAFE_INTEGER: u64 = 9_007_199_254_740_991;
type Mutation = (&'static str, fn(&mut ConfigDraft));
...
fn complete_config() -> ConfigDraft {
    ConfigDraft {
        version: 1,
        max_connections: Some(MAX_SAFE_INTEGER),
...
fn normalized_round_trip(config: ConfigDraft) -> (ConfigDraft, String) {
    let config = config.validate().expect("valid test configuration");
    let config = config.to_draft();
...
fn round_trips_rtmp_exec_profiles_as_data_only_lua() {
    ...
    let config = config.validate().expect("valid exec profile").to_draft();
...
fn automatic_response_headers_defaults_true_and_round_trips_both_values() {
    ...
    let mut config: ConfigDraft = serde_json::from_value(value).expect("omitted header policy");
...
fn rejects_non_utf8_paths_without_lossy_exposure() {
    ...
    let assert_render_error = |config: &ConfigDraft, expected: &str| {
        let message = render_lua(config)
            .expect_err("expected a Lua rendering error")
            .to_string();
    ...
    assert_render_error(
        &config,
        "management `management` has invalid `ui_dir`: path must be valid UTF-8",
    );
    ...
    assert_render_error(
        &listener,
        "listener `live` has invalid Unix socket `bind.path`: path must be valid UTF-8",
    );
    ...
    assert_render_error(
        &endpoint,
        "upstream pool `unix-backends` has invalid Unix socket `endpoints[].path`: path must be valid UTF-8",
    );
    ...
    assert_render_error(
        &recorder,
        "RTMP recorder `continuous` in application `live` of service `live` has invalid `root_directory`: path must be valid UTF-8",
    );
    ...
    *token_file_path = PathBuf::from(OsString::from_vec(b"/run/secrets/token-\xff".to_vec()));
    assert_render_error(
        &access,
        "HTTP access policy `web route 0` has invalid `token_file_path`: path must be valid UTF-8",
    );
    ...
    *root_directory = PathBuf::from(OsString::from_vec(b"/srv/www-\xff".to_vec()));
    assert_render_error(
        &static_files,
        "HTTP service `web` route 3 has invalid `action.static_files.root_directory`: path must be valid UTF-8",
    );
}
