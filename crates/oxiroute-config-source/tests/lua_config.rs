...
use oxiroute_config::{
    ...
    UpstreamAlgorithm, UpstreamEndpoint,
};
...
fn lua_error_from(source: &str) -> oxiroute_config_source::LuaConfigError {
...
fn applies_the_same_optional_admission_contract_to_json() {
    ...
    let omitted: oxiroute_config::ConfigDraft =
    ...
    let explicit_null: oxiroute_config::ConfigDraft =
    ...
    let numeric: oxiroute_config::ConfigDraft =
...
fn requires_explicit_tagged_listener_and_upstream_objects() {
    ...
    assert!(matches!(
        lua_error_from(&old_listener),
        oxiroute_config_source::LuaConfigError::Lua(_)
    ));
    ...
    for source in [
        ...
        let error = lua_error_from(&source);
        assert!(matches!(
            error,
            oxiroute_config_source::LuaConfigError::Lua(_)
        ));
...
fn rejects_unknown_protocols_and_algorithms() {
    let protocol_error = lua_error_from(&changed(
        "      protocol = \"http\",",
        "      protocol = \"sctp\",",
    ));
    assert!(matches!(
    ...
    let algorithm_error = lua_error_from(&changed(
        r#"      endpoints = {
        { type = "socket", address = "127.0.0.1:3000" },
        { type = "socket", address = "127.0.0.1:3001" },
      },
