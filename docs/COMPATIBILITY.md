...
| Capability | Status |
| --- | --- |
| Imported PEM certificate/key | partial: strict bounded direct-file and descriptor-relative Certbot loading creates immutable generations; declared DNS/IP identities must exactly match typed SANs with IPv4-mapped IPv6 canonicalized to IPv4, one TLS profile can select multiple exact/wildcard DNS SNI identities with an explicit default, and Certbot lineages are watched, but direct-file/config reload is absent |
...
| nginx | partial | `import_root` loads one complete include graph, emits one root terminal decision per occurrence, and merges strict HTTP and nginx-RTMP candidates while retaining deployment concerns. Bind lowering uses only first-loaded accepted exact/wildcard claims. Value-bearing certificate, htpasswd, bearer, upstream-TLS, and host-timezone overlays must be uniquely supplied and consumed before finalization. Bind-wide identical effective gzip `on`/`off`, level, concrete types, minimum length, HTTP version, `gzip_proxied off`, and vary policy lower natively with nginx defaults; unsupported proxied modes fail closed. Static `etag on|off` lowers. Proxy HTTP/1.0 or buffering defaults, active X-Accel controls, non-equivalent TLS session/cipher/DH/H2 policy, formatted logs, and cross-location static reroutes still fail closed. The HTTP fragment API remains available; the daemon binary provides offline report/preview only. |
