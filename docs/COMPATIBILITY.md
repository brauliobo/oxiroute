...
| HTTP/3 downstream/upstream | partial | The active `http3` reverse and `forward_http3` UDP listeners use separate bounded Quinn/H3 runtimes with TLS 1.3, `h3` ALPN, explicit SNI and trust roots, disabled migration/0-RTT, deadline/cancellation handling, response trailers, and no H1/H2 fallback under an H3 policy; broader conformance remains. |
...
| HTTP/3 reverse proxy | partial | `http3` is daemon-integrated through a separate UDP listener with validated HTTP routing, fixed/redirect/proxy actions, explicit `3/3` QUIC/H3 upstream pools with SNI/custom CA support, bounded request/response resources, trailers, safe retries, disabled migration/0-RTT, and generation drain; static files, cache, compression, upgrades, and broader conformance remain unsupported. |
