...
impl ChunkDeserializer {
    ...
    pub fn new() -> ChunkDeserializer {
        Self::new_with_limits(2147483647, 16777215)
    }

    /// Creates a deserializer with finite inbound chunk and assembled-message admission bounds.
    ...
    pub fn set_max_chunk_size(&mut self, new_size: usize) -> Result<(), ChunkDeserializationError> {
        ...
        if new_size == 0 || new_size > self.max_allowed_chunk_size {
            return Err(ChunkDeserializationError::ChunkSizeLimitExceeded {
                chunk_size: new_size,
                maximum: self.max_allowed_chunk_size,
            });
        }
    ...
    fn get_message_length(&mut self) -> Result<ParseStageResult, ChunkDeserializationError> {
        ...
        if length == 0 || length > self.max_message_size {
            return Err(ChunkDeserializationError::MessageLengthLimitExceeded {
