...
The current worktree has verified local evidence for importer provenance and deterministic
Varnish finalized/blocked reports, RTMP malformed-AMF rejection without panics and interleaved
fragmented-chunk decoding, UDP reload/session retention and drain behavior, and UI contract plus
desktop/mobile browser coverage. GitNexus reports zero import cycles. The Rust 1.97.1 format,
...
| HTTP/3 downstream/upstream | partial | The active `http3` reverse and `forward_http3` UDP listeners use separate bounded Quinn/H3 runtimes with TLS 1.3, `h3` ALPN, explicit SNI and trust roots, disabled/optional/required downstream client authentication with immutable bounded CA bundles and exact DNS/IP SAN checks, reverse routing/static/fixed/proxy responses, forward classic CONNECT and bounded absolute-form requests, response trailers, generation-owned graceful GOAWAY drain, disabled migration/0-RTT, deadline/cancellation handling, and no H1/H2 fallback under an H3 policy. Eligible reverse and forward GET/HEAD requests share bounded memory/persistent cache lookup, collapsed fills, revalidation, stale-if-error, purge, and admission. Client-auth failures close QUIC before H3 request bytes are processed; active-traffic drain evidence and broader conformance remain gates. Compression, upgrades, CONNECT-UDP, and broader forward conformance remain explicitly unsupported. |
...
| HTTP/3 forward proxy | partial | `forward_http3` is daemon-integrated through a separate UDP listener with TLS-level disabled/optional/required downstream client authentication, exact client DNS/IP SAN enforcement, authority-only classic CONNECT, bounded HTTP/HTTPS absolute-form forwarding, shared authorization/destination policy, bounded QUIC resources, fail-closed no-fallback coverage, and eligible memory/persistent GET/HEAD caching with revalidation, stale-if-error, collapsed fills, purge, and listener outcomes. Independent wire coverage proves a second cached request avoids origin contact; CONNECT-UDP and broader conformance remain unsupported. |
| HTTP/3 reverse proxy | partial | `http3` is daemon-integrated through a separate UDP listener with TLS-level disabled/optional/required downstream client authentication, exact client DNS/IP SAN enforcement, validated HTTP routing, fixed/redirect/static/proxy actions, explicit `3/3` QUIC/H3 upstream pools with SNI/custom CA support, bounded request/response resources, safe response framing and trailers, safe retries, implemented/tested graceful GOAWAY generation drain, disabled migration/0-RTT, no H1/H2 fallback, and bounded memory/persistent GET/HEAD caching with the shared transaction lifecycle. Independent wire coverage proves cache reuse without a second origin request; compression, upgrades, and broader conformance remain unsupported. |
...
| Response retries and passive ejection | stable for bounded configurable refused-stream, response-status, empty-response, response-timeout, and malformed-response retries plus per-pool passive observation, threshold, mark-down/up, backoff, recovery, metrics, and events; canonical/configuration and frontend controls include an explicit buffered-idempotent-request opt-in for intentionally idempotent `POST` APIs, while HAProxy import remains conservative and never infers that opt-in |
...
| Supervised master/worker listener adoption | partial: authenticated typed TCP, Unix, UDP, and QUIC/H3 descriptor adoption, worker status, initial UDP relay/H3 serving, drain, rollback, and crash handling are implemented and tested; eligible Linux `serve` configurations use supervision when the fixed launcher is installed (including the Arch package), while unsupported topologies, unpackaged installs without it, and non-Linux builds run direct; production supervised replacement/drain evidence for active UDP/H3 traffic remains |
...
| HLS H.264/AAC transmuxing and AES keys | partial: bounded canonical MPEG-TS HLS output, canonical variants, atomic storage, cleanup, and AES-128 key rotation are integrated; native nginx-RTMP `hls_variant` lowering, transcoding, and broader origin/auth policy remain absent |
...
| ACME DNS-01 and wildcard names | partial: bounded wildcard identifiers, DNS-01 TXT derivation, statically linked exact-name provider registration, bounded credentials and cooperative operation deadlines/cancellation, provider propagation hooks, exact confirmed cleanup, durable pending-cleanup journaling/recovery, atomic validated activation, and redacted status are implemented; provider calls are not preemptible, and provider deployments and live staging evidence remain |
...
| --- | --- | --- |
| OxiRoute Lua | partial | Aggregate/listener admission, Unix modes, downstream and route-local policy, nginx wildcard selectors, bounded headers/auth/cookies/static/gzip/logging, named server capacity and queueing, balancing, DNS, pool timeout/reuse, extended health policy, and RTMP chunk/fanout/push/recording naming policies compile into the runtime. Response buffering requires a positive route body cap and a bounded fixed-length origin response; supported bounded memory/persistent cache policies are runtime-integrated, while unsupported cache forms fail closed. RTMP file access logging uses the bounded fixed JSONL sink. |
