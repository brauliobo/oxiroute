...
mod unix {
    ...
    fn put_string(
        ...
        let length =
            u16::try_from(value.len()).map_err(|_| RtmpAutoPushError::TransportUnavailable)?;
    ...
    impl<'a> Reader<'a> {
        ...
        fn string(&mut self, maximum: usize) -> Result<String, RtmpAutoPushError> {
        ...
        fn array<const N: usize>(&mut self) -> Result<[u8; N], RtmpAutoPushError> {
            self.bytes(N)?
                .try_into()
                .map_err(|_| RtmpAutoPushError::TransportUnavailable)
        }

        fn u8(&mut self) -> Result<u8, RtmpAutoPushError> {
            let byte = *self
                .bytes
                .get(self.position)
        ...
        fn u16(&mut self) -> Result<u16, RtmpAutoPushError> {
            Ok(u16::from_be_bytes(self.array()?))
        }

        fn u32(&mut self) -> Result<u32, RtmpAutoPushError> {
            Ok(u32::from_be_bytes(self.array()?))
        }

        fn u64(&mut self) -> Result<u64, RtmpAutoPushError> {
            Ok(u64::from_be_bytes(self.array()?))
        }

        fn bytes(&mut self, length: usize) -> Result<Vec<u8>, RtmpAutoPushError> {
            let end = self
                .position
                .checked_add(length)
                .ok_or(RtmpAutoPushError::TransportUnavailable)?;
        ...
        fn finish(self) -> Result<(), RtmpAutoPushError> {
