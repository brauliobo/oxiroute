...
| HTTP/3 downstream/upstream | partial | The active `http3` reverse and `forward_http3` UDP listeners use separate bounded Quinn/H3 runtimes with TLS 1.3, `h3` ALPN, explicit SNI and trust roots, static/fixed/proxy responses where configured, response trailers, generation-owned graceful GOAWAY drain, disabled migration/0-RTT, deadline/cancellation handling, and no H1/H2 fallback under an H3 policy. A targeted reverse process test specifies reload/shutdown and no post-GOAWAY request; passing active-traffic drain evidence and broader conformance remain gates. Cache, compression, and upgrades remain explicitly unsupported. |
...
| Response retries and passive ejection | stable for bounded safe refused-stream/pre-send retries and per-pool passive observation, threshold, mark-down/up, backoff, recovery, metrics, and events; broader response-status retry policy remains |
...
| Vue 3 and build-time Pug UI | partial: responsive monitoring/topology observatory, RTMP broadcast desk, operations, certificate, bounded event-history, provenance, and bearer-unlocked canonical configuration workspaces are implemented with validation, Lua/candidate review, conflict handling, SSE refresh/reconnect, and pending-activation save reporting; native import editing and TLS-ALPN challenge selection are not exposed in the frontend |
| Management API | partial: loopback monitoring/topology, RTMP snapshot/detail/statistics/session controls, configured exact-ID recorder controls, authenticated config GET/validate/revision-checked durable PUT, listener/pool/server operations, generation actions, full bounded ACME lifecycle actions, process actions, cursor events/SSE, readiness/status/metrics, and authenticated stats administration are implemented; native import routes, durable event delivery, and broader API/UI workflows remain |
...
| Chunk formats 0-3 and extended timestamps | partial: `rml_rtmp` transport is active with a 1 MiB inbound chunk limit, a bounded service-configured assembled-message limit, configured acknowledgement window, and configured outbound chunk size announced on wire; exhaustive fragmentation/interleaving tests remain |
...
| Active stream and recording UI | partial: live recorder snapshots, exact-ID manual recorder controls, RTMP client statistics/drop controls, certificate lifecycle status/actions, bounded event history, and source-provenance views are implemented; native import editing and broader media controls remain |
...
| RTMP statistics/control API equivalents | partial: authenticated global/live/client statistics and revision-checked publisher/subscriber/session drop controls are integrated with bounded result shapes; native XML/form `rtmp_stat` and `rtmp_control` parity, redirects, and broader control coverage remain absent |
