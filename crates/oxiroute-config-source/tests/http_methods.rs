...
use oxiroute_config::{ConfigDraft, ForwardAccessMatcher, HttpRouteAction};
use serde_json::json;

fn authored_config() -> ConfigDraft {
...
fn route_methods(config: &ConfigDraft) -> &[String] {
    &config.http_services[0].routes[0].methods
}

fn cache_methods(config: &ConfigDraft) -> &[String] {
...
fn forward_methods(config: &ConfigDraft) -> &[String] {
...
fn serde_preserves_authored_methods_and_validation_canonicalizes_each_owner() {
    ...
    let canonical = authored.clone().validate().expect("valid HTTP tokens");
    let canonical = canonical.as_draft();
...
fn each_owner_rejects_invalid_http_tokens() {
    for invalid in [
        ...
        assert!(route.validate().is_err(), "route accepted {invalid:?}");
        ...
        assert!(cache.validate().is_err(), "cache accepted {invalid:?}");
        ...
        *methods = vec![invalid.into()];
        assert!(forward.validate().is_err(), "forward accepted {invalid:?}");
    }
}
...
fn cache_keeps_its_get_head_restriction_after_token_normalization() {
    for method in ["m-search", "x1", "foo!"] {
        ...
        assert!(config.validate().is_err(), "cache accepted {method}");
    }
}
