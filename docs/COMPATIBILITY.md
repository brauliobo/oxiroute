...
| HTTP/3 downstream/upstream | partial | The active `http3` reverse and `forward_http3` UDP listeners use separate bounded Quinn/H3 runtimes with TLS 1.3, `h3` ALPN, explicit SNI and trust roots, static/fixed/proxy responses where configured, response trailers, generation-owned graceful GOAWAY drain, disabled migration/0-RTT, deadline/cancellation handling, and no H1/H2 fallback under an H3 policy. Generation-owned drain is implemented and process-tested, including reload/shutdown and no post-GOAWAY request; active-traffic drain evidence and broader conformance remain gates. Cache, compression, and upgrades remain explicitly unsupported. |
...
| Weighted round robin | stable for canonical weighted selection with one bounded weight per server, deterministic health/capacity-aware runtime selection, topology/metrics exposure, and configuration/runtime/failure tests; native importer lowering and the frontend weight editor remain outside the current exposure |
...
| Response retries and passive ejection | stable for bounded configurable safe refused-stream/pre-send retries and per-pool passive observation, threshold, mark-down/up, backoff, recovery, metrics, and events; canonical/configuration and frontend controls plus the HAProxy connection-failure subset are covered, while broader response-status retry policy remains |
