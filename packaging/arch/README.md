...
`https://github.com/brauliobo/oxiroute/releases/download/v0.4.1/oxiroute-0.4.1.tar.gz`. The asset is a
deterministic archive of Git-tracked files with the `oxiroute-0.4.1/` prefix and without this
...
The v0.4.1 archive includes the supervised launcher and related workspace crates. Publish the release
...
```sh
./build-local.sh
./build-local.sh /path/to/oxiroute-0.4.1.tar.gz --cleanbuild
```
